...
from loss_func import CPA_Loss_init, CPA_Loss, proto_factor_cosine, tau_func, global_avg_proto, global_gaussian_proto

from server import communication # com_fc
import config
...
if __name__ == '__main__':
    ...
    loss_fun_init = CPA_Loss_init(
        class_counts=config.global_category_info,
        beta=args.loss_beta
    )
    ...
    for a_iter in range(args.iters): #
        ...
        if a_iter>=args.init_iter:
            loss_fun = loss_fun_refine
        else:
            loss_fun = loss_fun_init
        ...
        # init local proto
        if 'logit' in args.proto_type.lower():
            local_protos = torch.ones(4, 3, 3).to(device)
        else:
            local_protos = torch.ones(4, 3, 64).to(device)
        #
        ...
        for wi in range(args.wk_iters):
            ...
            for client_idx in range(client_num):
                ...
                # DET
                DET_stages[client_idx], train_loss_, train_loss_deputy_, train_acc_, train_f1_, train_auc_, local_proto = DET(
                    args, 
                    model_deputy, 
                    model, 
                    train_loader, 
            ...
            for test_idx, test_loader in enumerate(test_loaders):
                test_loss, test_acc, test_f1, test_auc  = test(args, models[test_idx], test_loader, loss_fun, device, local_protos[test_idx], global_proto)
            ...
            # 
            # print('finish collecting local proto')
            if 'avg' in args.global_proto_type.lower():
                global_proto = global_avg_proto(local_protos) ## 
            else:
                global_proto = global_gaussian_proto(local_protos)
            ...
            for client_idx in range(client_num):
                ...
                # print_cz('client {:d},\n cosine_score:\n{}'.format(client_idx, cosine_score), f=logfile)
                # print_cz('proto_factor:\n{}'.format(proto_factor), f=logfile)
        ...
        for valid_idx, valid_loader in enumerate(valid_loaders):
            valid_loss, valid_acc, valid_f1, valid_auc  = test(args, models[valid_idx], valid_loader, loss_fun, device, local_protos[valid_idx], global_proto)
